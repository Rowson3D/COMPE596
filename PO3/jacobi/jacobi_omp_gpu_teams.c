...
#define T_results(i, j) (T_results[(i)*(n_cells+2) + (j)])
...
#pragma omp target teams distribute collapse(2) \
    ...
    for (unsigned i = 1; i <= n_cells; i++)
      for (unsigned j = 1; j <= n_cells; j++)
        T_new(i, j) =
            0.25 * (T(i + 1, j) + T(i - 1, j) + T(i, j + 1) + T(i, j - 1));
    int M,N, _num_teams = 4; 
#pragma omp target teams /* num_teams(4) */ map(M,N,_num_teams)
    {
      M = omp_get_num_teams();
      int num_teams = 4;//_num_teams;
      omp_set_num_teams(num_teams);
      N = omp_get_num_teams();
    }
    
    printf("omp_get_num_teams initial default (streaming multiprocessors SMs): %d\n",M);
    printf("omp_set_num_teams set (streaming multiprocessors SMs): %d\n",N);
